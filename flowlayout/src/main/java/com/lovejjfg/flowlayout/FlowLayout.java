...
public class FlowLayout extends ViewGroup {
    ...
    public static final int FREE_EXACT_MODE = 3; //自由有上限的模式可以自由布局，但是限制每行最大的布局数量
    public static final int NORMAL_MODE = 4; //正常模式，不会自动消费剩余空间
...
//    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        finalWidth = (sizeWidth - mHorizontalSpacing * ((mDefalCount - 1))) / mDefalCount;
        ...
        for (int i = 0; i < count; i++) {
            ...
            } else if (mLayoutMode == FREE_MODE || mLayoutMode == NORMAL_MODE) {
                childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(sizeWidth, modeWidth == MeasureSpec.EXACTLY ? MeasureSpec.AT_MOST : modeWidth);
            } else {
                if (mLayoutMode == FREE_EXACT_MODE) {
                    childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(finalWidth, MeasureSpec.UNSPECIFIED);
                } else {
            ...
            if (child.getMeasuredWidth() > sizeWidth) {
                childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(sizeWidth, modeWidth == MeasureSpec.EXACTLY ? MeasureSpec.AT_MOST : modeWidth);
                child.measure(childWidthMeasureSpec, childHeightMeasureSpec);
            }
            ...
            if (mLayoutMode == FREE_EXACT_MODE) {
                if (mUsedWidth <= sizeWidth && mLine.getViewCount() < mDefalCount) {// 使用宽度小于总宽度，该child属于这一行。
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        if (!mNeedLayout || changed) {//没有发生改变就不重新布局
            ...
            for (int i = 0; i < linesCount; i++) {
                ...
                if (mLayoutMode == NORMAL_MODE) {
                    oneLine.layoutView(left, top, SPACE_NO_NEED);//布局每一行
